...
Every compared configuration must use the same prompt, the same repository commit, the same Archify skill and schema, the same time limit, and a clean candidate output path. Record the exact agent and model names. Preserve the unedited attempt 1 candidate and permit no post-hoc edits before verification.
