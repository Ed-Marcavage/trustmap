...
Author a fresh typed JSON diagram specification. Target the `showcase` quality profile; the external harness will validate the frozen candidate. Choose your own stable internal IDs and layout. Make the privacy boundary and restricted identity path visually distinct from the ordinary analytics path.
