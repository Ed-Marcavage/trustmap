...
- Omit `meta.engineering_profile` by default. Region, cluster, and security boundary wording do not by themselves enable it. Enable `deployment-ownership` only when the user explicitly asks for a production deployment topology, ownership handoff, or fail-closed deployment review and the source facts are known. Enable `contract-security` only when the user explicitly asks for a threat-model or fail-closed security review and the guards, scopes, and upgrade admins are known facts: it requires a trust boundary, every contract in exactly one trust-boundary, every role in a privilege-domain, a guard on every trust-boundary crossing (the literal `none` is explicit), a label and guard on every role→contract edge, a guard on every oracle read, and an upgrade-domain plus a role admin edge for every component with the authored fact `upgradeable: true` (never inferred from tags or names). Once enabled, must not remove the engineering profile merely to pass validation; repair the facts or report the diagnostics truthfully.
