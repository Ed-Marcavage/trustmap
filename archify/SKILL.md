...
3. Artifact first: after those reads finish, the next tool action must write the candidate. Write the candidate before inspecting renderer internals. Do not plan exact coordinates in prose, compare several layouts, inspect another example, or run CLI help first. Start with one clear main path, short side branches, 180–220 px horizontal spacing, 140–180 px branch spacing, sparse relationship labels, and at most 12 primary nodes; omit optional cards, views, and tags until validation passes. Set `meta.quality_profile` to `"showcase"` unless the user explicitly asks for a dense `standard` engineering map. Start with automatic routes and labels. Do not add `via`, `channelX`, `channelY`, or `labelAt` before a diagnostic explicitly calls for one; add at most one diagnosed geometry control per repair. Fresh authorship means new stable IDs, domain wording, and layout. When the requested roles naturally match an example, preserve the accurate topology instead of inventing a less accurate difference merely to look novel.
   - Lifecycle only: phase columns `0..4` occupy the main rail; event and outcome columns `0..2` align under phase columns `2..4`. Put approval under execution at event column `0` and blocked-under-review at event column `1`. A recoverable state uses `type: "failure"` plus a real transition back to the active state; a card or view saying “retry” is not topology. Do not add reverse resume transitions for waiting states unless the requested lifecycle explicitly requires them.
4. Run `node bin/archify.mjs validate <type> <candidate.json> --quality showcase --json` (or `standard` when the user explicitly wants a dense engineering map). Run it again after every candidate edit and immediately before handoff, even when an earlier iteration passed. A receipt with only 4 artifact checks is basic validation, never showcase acceptance; a showcase pass must report all 9 artifact checks with 0 composition errors and 0 warnings. If the candidate omits or misspells the exact `meta.quality_profile` field, fix that field before any geometry repair. A passing final validation freezes the candidate: never edit it afterward. When the requested output is a delivered HTML artifact, `deliver` is the final acceptance command: run `node bin/archify.mjs deliver <type> <candidate.json> <output.html> --quality showcase --json` because it revalidates those frozen bytes before committing the output. A non-zero exit can never be described as success.
5. If validation fails, change only the diagnosed subject using its `supportedFixes`, then rerun. Continue focused correction while the objective error count reaches a new minimum. If two consecutive rounds do not improve that best count, or only trade a cleared error for a new one, stop and report the unresolved diagnostics truthfully.
