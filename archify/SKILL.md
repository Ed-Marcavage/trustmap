...
4. Run `node bin/archify.mjs validate <type> <candidate.json> --quality showcase --json` (or `standard` when the user explicitly wants a dense engineering map). Run it again after the final candidate edit, even when an earlier iteration passed.
